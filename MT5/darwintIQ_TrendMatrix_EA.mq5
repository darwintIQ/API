...
string Lower(const string s){ return StringToLower(s); }
...
// Prefer "dir", but accept "direction"/"bias" when API changes naming
bool JsonGetDirLocal(const string json,int from,string &outVal){
   if(JsonGetStringLocal(json,from,"dir",outVal)) return true;
   if(JsonGetStringLocal(json,from,"direction",outVal)) return true;
   if(JsonGetStringLocal(json,from,"bias",outVal)) return true;
   return false;
}

// Prefer "strength", but accept "score"/"slope" if provided
bool JsonGetStrengthLocal(const string json,int from,double &outNum){
   if(JsonGetNumberLocal(json,from,"strength",outNum)) return true;
   if(JsonGetNumberLocal(json,from,"score",outNum)) return true;
   if(JsonGetNumberLocal(json,from,"slope",outNum)) { outNum=MathAbs(outNum); return true; }
   return false;
}
...
/* ============================ JSON parse ============================ */
int FindTrendBase(const string body){
   int snapPos = StringFind(body,"\"snapshot\"",0);
   if(snapPos<0) return 0;
   int dataPos = StringFind(body,"\"data\"",snapPos);
   if(dataPos<0) return snapPos;
   return dataPos;
}
...
bool ParseTrendMap(const string body){
   int basePos  = FindTrendBase(body);
   int trendPos = StringFind(body,"\"trend\"",basePos);
   if(trendPos<0 && basePos!=0)
      trendPos = StringFind(body,"\"trend\"",0);
   ...
   #define EXTRACT_TF(TF,dirVar,strVar) {                           \
      ...
      if(p>=0){ string d=""; double s=0;                             \
         JsonGetDirLocal(body,p,d);                                  \
         JsonGetStrengthLocal(body,p,s);                             \
         dirVar=(d==""?"Ranging":d);                                 \
   ...
   string c="";  if(!JsonGetStringLocal(body,basePos,"consensus",c)) JsonGetStringLocal(body,0,"consensus",c);
   g_consensus=(c==""?"NEUTRAL":c);
   string ot="";
   if(!JsonGetStringLocal(body,0,"opentime",ot)) {
      JsonGetStringLocal(body,basePos,"time",ot);
   }
   g_asOf=ot;
...
bool FetchLatest(){
   ...
   string url=InpApiBaseUrl + "/trendmatrix?symbol="+sym+"&latest=1";
