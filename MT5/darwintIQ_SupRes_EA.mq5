...
input string InpApiBaseUrl     = "https://api.darwintIQ.com/v1";
input string InpApiToken       = "YOUR_API_TOKEN";
...
// API options (latest shape)
input bool   InpEnsureReg       = true;  // fallback to most recent snapshot that has regression
input int    InpEnsureRegWindowMin = 360; // lookback window for ensure_reg (minutes)
...
datetime ParseIsoTime(const string iso){
   string s=iso;
   StringReplace(s,"-",".");
   return StrToTime(s); // MT5 can parse "YYYY.MM.DD HH:MM:SS"
}

// If API wraps payload in "snapshot", use that as base search position
int FindSnapshotBase(const string body){
   int snapPos = StringFind(body,"\"snapshot\"",0);
   if(snapPos<0) return 0;
   int objStart = StringFind(body,"{",snapPos);
   if(objStart<0) return snapPos;
   return objStart;
}
...
bool ParseRegBlock(const string body,const string key,int startPos){
   // locate "reg" first
   int regPos = StringFind(body,"\"reg\"",startPos);
...
bool ParseSwingSide(const string body,const string side,int startPos){
   int swPos = StringFind(body,"\"swing\"",startPos);
...
string BuildHeaders(){
   string h="";
   h += "Authorization: Bearer "+InpApiToken+"\r\n";
...
{
   string url = InpApiBaseUrl + "/supres?symbol=" + sym + "&latest=1";
   if(InpEnsureReg){
      url += "&ensure_reg=1&windowMin=" + IntegerToString(InpEnsureRegWindowMin);
   }
   ...
   // Expect shape: { status, symbol, latest, opentime, meta, snapshot:{ cur:[...], reg:{mid,up,dn}, swing:{top,bot}} }
   int basePos = FindSnapshotBase(body);

   // 1) symbol / opentime (root preferred, fallback to snapshot/meta)
   string symOut="";
   ...
   else if(JsonGetStringLocal(body,basePos,"bar_open",ot)) g_lastOpenTime=ot;

   // 2) cur levels
   ParseCurLevels(body,basePos);

   // 3) regression
   bool okMid = ParseRegBlock(body,"mid",basePos);
   bool okUp  = ParseRegBlock(body,"up",basePos);
   bool okDn  = ParseRegBlock(body,"dn",basePos);
   g_hasReg = (okMid && okUp && okDn);

   // 4) swing
   bool okTop = ParseSwingSide(body,"top",basePos);
   bool okBot = ParseSwingSide(body,"bot",basePos);
...
void RenderAllChartsOrCurrent(){
   ...
   // MT5: iterate open charts using ChartFirst/ChartNext
   long cid = ChartFirst();
   while(cid != -1)
   {
